...
class BLIP_Retrieval(nn.Module):
    ...
    def forward(self, image, caption, alpha, idx):
        ...
        text = self.tokenizer(caption, padding='max_length', truncation=True, max_length=180, 
        ...
        sign_feat = F.normalize(self.text_proj(text_output.last_hidden_state[:,1,:]),dim=-1) 
        ...
        img_att = (img_att.softmax(dim=1)).expand(-1,-1,self.text_width)
        img_word = img_word.mul(img_att)
        
        # text 단위 att
        text_att = self.tanh(self.att_layer_v(text_word))
        text_att = text_att @ self.W_v
        text_att = (text_att.softmax(dim=1)).expand(-1,-1,self.text_width)
        ...
        img_word_sum = F.normalize(self.vision_proj(img_word_sum),dim=-1)
        text_word_sum = F.normalize(self.text_proj(text_word_sum), dim=-1)
        phrase_sum = F.normalize(self.text_proj(phrase_sum), dim=-1)
        
        image_feat = image_feat*(1-0.05) + img_word_sum*0.05
        text_feat = text_feat*(1-0.25) + text_word_sum*0.25        
        ...
        return loss_ita, loss_itm, symbol_simloss
